...
```bash
# Sonnet（デフォルト、バランス型）
mao start --interactive "タスク"

# Opus（高性能、複雑なタスク向け）
mao start --interactive --model opus "複雑なタスク"

# Haiku（高速、シンプルなタスク向け）
mao start --interactive --model haiku "簡単なタスク"
```
...
## 比較: 通常モード vs インタラクティブモード

| 項目 | 通常モード | インタラクティブモード |
|------|-----------|----------------------|
| 起動 | `mao start` | `mao start --interactive` |
| マネージャー | なし | あり（対話可能） |
| タスク分配 | 自動 | マネージャー経由 |
| 柔軟性 | 低 | 高 |
| 学習曲線 | 簡単 | 中程度 |
| 用途 | シンプルなタスク | 複雑なタスク、計画が必要 |
